...
public class CSVHelperTest {
	...
	public void write() {
		...
		rowList.add(new CSVRow(new StringCellInfo[] {
...
//		assertEquals("\"", rowList.get(0).get(2).getValue()); TODO bag
...
//		assertEquals("t\"st", rowList.get(0).get(7).getValue()); TODO bag
		assertEquals("t\rst", rowList.get(0).get(8).getValue());
//		assertEquals("t\nst", rowList.get(0).get(9).getValue()); TODO bag
	}
	...
	public void read2() {
		CSVHelper helper = new CSVHelper();
		List<CSVRow> rowList = new ArrayList<>();
		helper.read(new File("test.csv"), row -> {
			rowList.add(row);
		});
		assertEquals("test", rowList.get(0).get(0).getValue());
		assertEquals(",", rowList.get(0).get(1).getValue());
